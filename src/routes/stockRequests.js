...
router.put('/:id/approve', async (req, res) => {
  try {
    const request = await StockRequest.findByIdAndUpdate(req.params.id, { status: 'APPROVED' }, { new: true }).populate('product purchaseOrder');
    if (!request) return res.status(404).json({ message: 'Stock request not found' });
    res.json(request);
  } catch (err) {
    res.status(500).json({ message: err.message });
...
router.put('/:id/po-created', async (req, res) => {
  try {
    const request = await StockRequest.findByIdAndUpdate(req.params.id, { status: 'PO_CREATED' }, { new: true }).populate('product purchaseOrder');
    if (!request) return res.status(404).json({ message: 'Stock request not found' });
    res.json(request);
  } catch (err) {
    res.status(500).json({ message: err.message });
...
router.put('/:id/fulfill', async (req, res) => {
  try {
    const request = await StockRequest.findByIdAndUpdate(req.params.id, { status: 'FULFILLED' }, { new: true }).populate('product purchaseOrder');
    if (!request) return res.status(404).json({ message: 'Stock request not found' });
    res.json(request);
  } catch (err) {
    res.status(500).json({ message: err.message });
